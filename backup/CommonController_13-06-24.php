                    ...
                    if (in_array('paper-list', $url_data)) { //check url has permission or not
                        ...
                        if ($user_data->u_role_id == '1') { //Super Admin or Council Admin
                            ...
                            if ($user_data->is_direct == '0') { //Not a special examiner
                                ...
                                $paper_list = TheorySubject::where('is_active', 1)->whereIn('paper_id_pk', $allPaperIds)->get();
                                //return $paper_list;
                            } else { //Special Examiner
                                ...
                                $paper_list = TheorySubject::where('is_active', 1)->whereIn('paper_id_pk', $externalPaperIds)->get();
                            }
                        }
            ...
            if ($token_check) {  // check the token is expire or not
                ...
                if (sizeof($role_url_access_id) > 0) {
                    ...
                    if (in_array('active-session', $url_data)) { //check url has permission or not
                        ...
                        $current_session = DB::table('wbscte_other_diploma_session_tbl')->select('session_name')->where('session_active', 1)->get()->pluck('session_name');
                        $sessions = [];
                        foreach ($current_session as $current_sessions) {
                            $financialYears = getFinancialYear($current_sessions, $type);
                            $sessions = array_merge($sessions, $financialYears); 
                        }
