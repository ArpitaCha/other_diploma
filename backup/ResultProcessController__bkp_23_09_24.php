    ...
    public function resultProcessScriptStep1(Request $request)
    {
        ...
        if (sizeof($resultData) > 0) {
            foreach ($resultData as $key => $row) {
                ...
                $SUB6_OB_MK = (int)$row['SUB6_TH_INT_MK_' . $year . ''] + (int)$row['SUB6_TH_INT_ATD_MK_' . $year . ''] + (int)$row['SUB6_TH_EXT_MK_' . $year . ''];
                $SUB7_OB_MK = (int)$row['SUB7_TH_INT_MK_' . $year . ''] + (int)$row['SUB7_TH_INT_ATD_MK_' . $year . ''] + (int)$row['SUB7_TH_EXT_MK_' . $year . ''];
                $SUB8_OB_MK = (int)$row['SUB8_TH_INT_MK_' . $year . ''] + (int)$row['SUB8_TH_INT_ATD_MK_' . $year . ''] + (int)$row['SUB8_TH_EXT_MK_' . $year . ''];
                $SUB9_OB_MK = (int)$row['SUB9_SES_INT_MK_' . $year . ''] + (int)$row['SUB9_SES_EXT_MK_' . $year . ''];
                $SUB10_OB_MK = (int)$row['SUB10_SES_INT_MK_' . $year . ''] + (int)$row['SUB10_SES_EXT_MK_' . $year . ''];
                $SUB11_OB_MK = (int)$row['SUB11_SES_INT_MK_' . $year . ''] + (int)$row['SUB11_SES_EXT_MK_' . $year . ''];
                $SUB12_OB_MK = (int)$row['SUB12_SES_INT_MK_' . $year . ''] + (int)$row['SUB12_SES_EXT_MK_' . $year . ''];
                $SUB13_OB_MK = (int)$row['SUB13_SES_INT_MK_' . $year . ''] + (int)$row['SUB13_SES_EXT_MK_' . $year . ''];
                $SUB14_OB_MK = (int)$row['SUB14_SES_INT_MK_' . $year . ''] + (int)$row['SUB14_SES_EXT_MK_' . $year . ''];
    ...
    public function resultProcessScriptStep2(Request $request)
    {
        ...
        if (sizeof($resultData) > 0) {
            foreach ($resultData as $key => $row) {
                ...
                $SUB9_INT_OB_MK = $row['SUB9_SES_INT_MK_' . $year . ''];
                $SUB9_EXT_OB_MK = $row['SUB9_SES_EXT_MK_' . $year . ''];

                $SUB10_INT_OB_MK = $row['SUB10_SES_INT_MK_' . $year . ''];
                $SUB10_EXT_OB_MK = $row['SUB10_SES_EXT_MK_' . $year . ''];

                $SUB11_INT_OB_MK = $row['SUB11_SES_INT_MK_' . $year . ''];
                $SUB11_EXT_OB_MK = $row['SUB11_SES_EXT_MK_' . $year . ''];

                $SUB12_INT_OB_MK = $row['SUB12_SES_INT_MK_' . $year . ''];
                $SUB12_EXT_OB_MK = $row['SUB12_SES_EXT_MK_' . $year . ''];

                $SUB13_INT_OB_MK = $row['SUB13_SES_INT_MK_' . $year . ''];
                $SUB13_EXT_OB_MK = $row['SUB13_SES_EXT_MK_' . $year . ''];

                $SUB14_INT_OB_MK = $row['SUB14_SES_INT_MK_' . $year . ''];
                $SUB14_EXT_OB_MK = $row['SUB14_SES_EXT_MK_' . $year . ''];
                $credit = "";
                //dd($SUB1_OB_MK);
    ...
    public function resultProcessScriptStep6(Request $request)
    {
        ...
        if (sizeof($resultData) > 0) {
            foreach ($resultData as $key => $row) {
                ...
                $SUB9_INT_OB_MK = (int)$row['SUB9_SES_INT_MK_' . $year . ''];
                $SUB9_EXT_OB_MK = (int)$row['SUB9_SES_EXT_MK_' . $year . ''];
                $SUB9_OB_MK = (int)($SUB9_INT_OB_MK + $SUB9_EXT_OB_MK);

                $SUB10_INT_OB_MK = (int)$row['SUB10_SES_INT_MK_' . $year . ''];
                $SUB10_EXT_OB_MK = (int)$row['SUB10_SES_EXT_MK_' . $year . ''];
                $SUB10_OB_MK = (int)($SUB10_INT_OB_MK + $SUB10_EXT_OB_MK);

                $SUB11_INT_OB_MK = (int)$row['SUB11_SES_INT_MK_' . $year . ''];
                $SUB11_EXT_OB_MK = (int)$row['SUB11_SES_EXT_MK_' . $year . ''];
                $SUB11_OB_MK = (int)($SUB11_INT_OB_MK + $SUB11_EXT_OB_MK);

                $SUB12_INT_OB_MK = (int)$row['SUB12_SES_INT_MK_' . $year . ''];
                $SUB12_EXT_OB_MK = (int)$row['SUB12_SES_EXT_MK_' . $year . ''];
                $SUB12_OB_MK = (int)($SUB12_INT_OB_MK + $SUB12_EXT_OB_MK);

                $SUB13_INT_OB_MK = (int)$row['SUB13_SES_INT_MK_' . $year . ''];
                $SUB13_EXT_OB_MK = (int)$row['SUB13_SES_EXT_MK_' . $year . ''];
                $SUB13_OB_MK = (int)($SUB13_INT_OB_MK + $SUB13_EXT_OB_MK);

                $SUB14_INT_OB_MK = (int)$row['SUB14_SES_INT_MK_' . $year . ''];
                $SUB14_EXT_OB_MK = (int)$row['SUB14_SES_EXT_MK_' . $year . ''];
                $SUB14_OB_MK = (int)($SUB14_INT_OB_MK + $SUB14_EXT_OB_MK);
    ...
    public function resultProcessScriptStep7(Request $request)
    {
        ...
        if (sizeof($resultData) > 0) {
            foreach ($resultData as $key => $row) {
                ...
                $SUB13_PASS_MK = (!is_null($row['SUB13_ID'])) ? $paper->clone()->select('paper_pass_marks')->find($row['SUB13_ID']) : NULL;
                $SUB14_PASS_MK = (!is_null($row['SUB14_ID'])) ? $paper->clone()->select('paper_pass_marks')->find($row['SUB14_ID']) : NULL;
                //echo($SUB1_PASS_MK.',--2--'.$SUB2_PASS_MK. ',--3--'.$SUB3_PASS_MK.',--4--'.$SUB4_PASS_MK.',--5--'.$SUB5_PASS_MK.',--6--'.$SUB6_PASS_MK.',--7--'.$SUB7_PASS_MK.',--8--'.$SUB8_PASS_MK.',--9--'.$SUB9_PASS_MK.',--10--'.$SUB10_PASS_MK.',--11--'.$SUB11_PASS_MK.',--12--'.$SUB12_PASS_MK.',--13--'.$SUB13_PASS_MK.',--14--'.$SUB14_PASS_MK);
                ...
                (int)$row["SUB5_OB_MK"] +
                (int)$row["SUB6_OB_MK"] + (int)$row["SUB7_OB_MK"] +
                (int)$row["SUB8_OB_MK"] + (int)$SUB9_OB_MK + (int)$SUB10_OB_MK + 
                (int)$SUB11_OB_MK + (int)$SUB12_OB_MK + (int)$SUB13_OB_MK + (int)$SUB14_OB_MK;
                //echo $GT."----";
                ...
                if ($creditCount == $paperCount) {
                    //print_r($paperCount."--".$creditCount);exit();
                    Result::where('REG_NO', $row['REG_NO'])
                            ->update([
                                'GRAND_TOTAL' => $GT,
                                'RESULT' => 'Q',
                ...
                }else if(($paperCount - $creditCount) > 2 ){
                    Result::where('REG_NO', $row['REG_NO'])
                    ->update([
                        'GRAND_TOTAL' => $GT,
                        'RESULT' => 'X',
                ...
                }else if(($paperCount - $creditCount >= 1) && ($paperCount - $creditCount <= 2)){
                    Result::where('REG_NO', $row['REG_NO'])
                            ->update([
                                'GRAND_TOTAL' => $GT,
                                'RESULT' => 'B',
