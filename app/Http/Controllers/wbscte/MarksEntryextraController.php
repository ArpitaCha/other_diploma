...
use App\Http\Resources\wbscte\MarksEntrylistExtraResource;
use App\Models\wbscte\Paper;
use Barryvdh\DomPDF\Facade\Pdf;

class MarksEntryextraController extends Controller
{
                        ...
                        if (in_array('marks-entry-list', $url_data)) {
                            ...
                            if ($validated->fails()) {
                                ...
                                $marks_list = Attendance::where(['attr_sessional_yr' => $session_yr, 'att_sem' => $semester, 'att_inst_id' => $inst_id, 'att_course_id' => $course_id, 'att_paper_id' => $paper_id, 'att_paper_type' => $paper_type, 'att_paper_entry_type' => $subject_entry_type])
                                    ->with('marks:id,att_id,marks,is_final_submit,internal_attendance_marks,internal_viva_marks,internal_class_test_marks', 'paperMarks:paper_id_pk,paper_full_marks,paper_internal_marks,paper_external_marks,paper_sessional_internal_viva_marks,paper_sessional_internal_class_test_marks,paper_sessional_internal_attendance_marks,paper_internal_attendance_marks,paper_internal_theory_class_test_marks,paper_internal_viva_marks', 'student:student_reg_no,student_fullname')->orderBy('att_reg_no', 'asc')
                                    ->get();
                                //  return $marks_list;
                                    if ($marks_list->where('is_final_submit', 1)->count()) {
                                        // Proceed with fetching paper marks and constructing response
                                            ...
                                            if ($paper_marks) {
                                                if ($paper_type == 1) {
                                                    $max_marks = [
                                                        ...
                                                        'viva_marks' =>$paper_marks->internal_viva_marks,
                                                        'class_test_marks'=>$paper_marks->internal_class_test_marks,
                                        ...
                                        if (sizeof($marks_list) > 0) {
                                            $response = [
                                                'error' => false,
                                                'message' => 'Data found',
                                                'marks' => MarksEntrylistExtraResource::collection($marks_list),
                                                'max_marks' => $max_marks
                                            ];
                                            return response()->json($response, 200);
                                        } else {
                        ...
                        try {
                            ...
                            foreach ($marks_list as $marks) {
                                ...
                                if ($validated->fails()) {
                                    ...
                                    $class_test_marks = $marks['class_test_marks'];
                                    $std_marks = (int)$viva_marks + (int)$class_test_marks +(int)$attendance_marks;
    ...
    public function marksPdf(Request $request)
    {
        ...
        $finalList = $marks_list->map(function ($single, $key) {
            ...
            return [
                ...
                'paper_internal_viva_marks' => $single->paperMarks->paper_internal_viva_marks,
                ...
                'internal_attendance_marks' => $single->internal_attendance_marks ?? '0',
                'internal_viva_marks' => $single->internal_viva_marks ?? '0',
                'internal_class_test_marks' => $single->internal_class_test_marks ?? '0',
                'total_marks' =>  (int)$single->internal_attendance_marks +  (int)$single->internal_class_test_marks +  (int)$single->internal_viva_marks,
        ...
        $pdf = Pdf::loadView('exports.marks_extra', [
            'marks' => $marks,
            'students' => $finalList,
