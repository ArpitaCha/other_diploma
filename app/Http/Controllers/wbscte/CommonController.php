            ...
            if ($token_check) {  // check the token is expire or not
                ...
                if (sizeof($role_url_access_id) > 0) {
                    ...
                    if (in_array('all-theory-paper', $url_data)) { //check url has permission or not
                        ...
                        $paper_res = $paper_list->orderBy('paper_id_pk', 'DESC')->get();
        ...
        if ($request->header('token')) {
            ...
            if ($token_check) {  // check the token is expire or not
                ...
                if (sizeof($role_url_access_id) > 0) {
                    ...
                    if (in_array('examiner-notexist-institute', $url_data)) {
        ...
        if ($request->header('token')) {
            ...
            if ($token_check) {  // check the token is expire or not
                ...
                if (sizeof($role_url_access_id) > 0) {
                    ...
                    if (in_array('create-user', $url_data)) { //check url has permission or not
                        $validated = Validator::make($request->all(), [
                            'username' => ['required'],
                            'fullname' => ['required'],
                            'role' => ['required'],
        ...
        if ($request->header('token')) {
            ...
            if ($token_check) {  // check the token is expire or not
                ...
                if (sizeof($role_url_access_id) > 0) {
                    ...
                    if (in_array('create-paper', $url_data)) { //check url has permission or not
                        $validated = Validator::make($request->all(), [
                            'course_id' => ['required'],
                            'inst_id' => ['required'],
                            'paper_code'=> ['required'],
        ...
        if ($request->header('token')) {
            ...
            if ($token_check) {  // check the token is expire or not
                ...
                if (sizeof($role_url_access_id) > 0) {
                    ...
                    if (in_array('create-course', $url_data)) { //check url has permission or not
                        $validated = Validator::make($request->all(), [
                            'course_code' => ['required'],
                            'inst_id' => ['required'],
                            'course_name' => ['required'],
        ...
        if ($request->header('token')) {
            ...
            if ($token_check) {  // check the token is expire or not
                ...
                if (sizeof($role_url_access_id) > 0) {
                    ...
                    if (in_array('create-institute', $url_data)) { //check url has permission or not
                        $validated = Validator::make($request->all(), [
                            'institute_code' => ['required'],
                            'institute_name' => ['required'],
                            'institute_address' => ['required'],
                ...
                if ($token_check) {  // check the token is expire or not
                  ...
                  //  dd($user_data);
                    ...
                    if (sizeof($role_url_access_id) > 0) {
                        ...
                        if (in_array('count-dashboard', $url_data)) { //check url has permission or not
                            $validated = Validator::make($request->all(), [
                                // 'inst_id' => ['required'],
                                'sessionYear' => ['required'],
                            ]);
                ...
                if ($request->header('token')) {
                    ...
                    if ($token_check) {  // check the token is expire or not
                        ...
                        if (sizeof($role_url_access_id) > 0) {
                            ...
                            $url_data = array_column($urls, 'url_name');
                            if (in_array('eligibility-list', $url_data)) { 
